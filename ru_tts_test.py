...
def wave_file(filename, pcm, channels=1, rate=24000, sample_width=2):
    print(f"Сохраняю файл {filename}...")
...
{cleaned_dialogue}"""
    ...
    print(f"\nГенерирую часть {part_number}...")
    print(f"Персонажи: {char1} ({voice1}) и {char2} ({voice2})")
...
def process_dialogue():
    try:
        ...
        print("\nОшибка: Файл dialogue.txt не найден!")
        return
    ...
    for i, dialogue_pair in enumerate(dialogues):
        # ЕДИНСТВЕННОЕ ИЗМЕНЕНИЕ - проверка temp файла
        ...
        if char1 and char2:
            ...
            print(f"\nОбработка диалога между {char1} и {char2}")
    ...
    if temp_files:
        ...
        print(f"\nГотово! Финальное аудио сохранено в файл: {final_filename}")
        return final_filename
    else:
        print("\nНе удалось создать аудио файлы")
